...
fork(void)
{
  ...
  if(uvmcopy(p->pagetable,np->pagetable, p->sz,&p->heapvma) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          // Found one.
          pid = pp->pid;
          if(addr != 0 && copyout(p->pagetable,&p->heapvma, addr, (char *)&pp->xstate,
                                  sizeof(pp->xstate)) < 0) {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
...
either_copyout(int user_dst, uint64 dst, void *src, uint64 len)
{
  struct proc *p = myproc();
  if(user_dst){
    return copyout(p->pagetable, &p->heapvma,dst, src, len);
  } else {
...
either_copyin(void *dst, int user_src, uint64 src, uint64 len)
{
  struct proc *p = myproc();
  if(user_src){
    return copyin(p->pagetable, &p->heapvma, dst, src, len);
  } else {
